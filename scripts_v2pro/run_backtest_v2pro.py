...
from src_v2pro.backtest import run_backtest_v2pro, save_outputs


def _algo_fp(root: Path) -> str:
    h = hashlib.sha256()
    paths = [root / "config_v2_pro.yaml", root / "scripts_v2pro" / "run_backtest_v2pro.py"]
    paths.extend(sorted((root / "src_v2pro").glob("*.py")))
    for p in paths:
        if p.exists():
...
def main():
    ...
    p.add_argument("--export", default="reports/backtest_v2pro")
    args = p.parse_args()
    ...
    summary, trades, equity = run_backtest_v2pro(ROOT, args.symbol, args.interval, args.date_from, args.date_to, args.cash)
